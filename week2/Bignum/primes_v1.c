...
int main(){
  ...
  */
  ...
  /* int BN_generate_prime_ex2(BIGNUM *ret, 
                               int bits                                 [EXPLANATION: minimum length of prime numbers], 
                               int safe                                 [EXPLANATION: prime number is safe if (p-1)/2 is also a prime], 
                               const BIGNUM *add, const BIGNUM *rem     [EXPLANATION: add,rem --> p % add == rem
                                                                         if rem is NULL --> rem = 1
                                                                         if rem is NULL and safe is true --> rem = 3 and must be multiple of 4],
  ...
  */
  ...
  if(BN_is_prime_ex(prime1,8,NULL,NULL)){   //the second parameter is the number of tests/checks over the prime number: usually it is 8 or 16
    printf("Yes, it's a prime\n");
  }
  else{
    printf("No, it isn't a prime\n");
  }
  /*Alternative function: Bn_check_prime(prime1, ctx, cb)   USING CTX */


  BN_set_word(prime2,128);  //we can transform un unsigned long in a big number representation, and so on we can control if this number (128) is a prime number or not
