...
int main(void) {
    ...
    */
    ...
    unsigned char encrypted_data[RSA_size(keypair)];    //as many bit as the key lenght


    if((encrypted_data_len = RSA_public_encrypt(strlen(msg)+1, msg, encrypted_data, keypair, RSA_PKCS1_OAEP_PADDING)) == -1) 
            handle_errors();    /*the first paramter is the length of the message plus 1 because we want to encrypt also the new line,
                                  the second parameter is the messag to encrypt,
                                  the third one is destination,
                                  the fourth is private key,
                                  the fifth is padding       */
    ...
    printf("Encrypted message written to file.\n");     //to see the content of out.bin you have to write "hexdump out.bin" on terminal
                                                        //writing "ls -la" on terminal you can see the size of out.bin (in this case it is equal to 256,
                                                        //that is exactly the length of the key)
